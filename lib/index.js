...
const createApplication = (commander) => {
  ...
  questions.askMany({
    ...
    createConfigApp(commander, answer);
    createConfigApi(commander, answer);
    installNpm();
  });
}
...
const createConfigApp = (commander, answer) => {
  let configText = '';
  ...
  fs.writeFileSync(currentDir() + '/config/app.js', configText);

  console.log('config.app.js file created successfully.');
  console.log();
}

const createConfigApi = (commander, answer) => {
  let apiKey = generateApiKey();

  let configText = '';
  configText += 'module.exports = {\n';
  configText += '  key: \'' + apiKey + '\'\n';
  configText += '};';

  fs.writeFileSync(currentDir() + '/config/api.js', configText);

  console.log('config.api.js file created successfully.');
  console.log();
}
